...
Original file is located at
    https://colab.research.google.com/drive/1xmBbAp_prPRBuO1C4iK8CciFpT6Qyu3Z
"""
...
def classify_land_use(image_data):
    ...
    for (id, label, prob) in decoded_preds:
        # Filter for relevant land-type predictions
        if label in ['valley', 'volcano', 'lakeside', 'promontory', 'seashore', 'geyser', 'alp', 'meadow']:
            land_types.append((label.title(), f"{(prob*100):.1f}%"))

    if not land_types:
        # Fallback if no specific land types are found
        land_types.append((decoded_preds[0][1].title(), f"{(decoded_preds[0][2]*100):.1f}%"))

    return land_types[0] # Return the top prediction
...
def get_smart_recommendation(degradation_percent, land_use_type):
    ...
    # For this demo, we'll simplify the MobileNetV2 output
    if 'valley' in land_use_type.lower() or 'meadow' in land_use_type.lower():
...
if st.session_state.analysis_results:
    ...
    # We'll create a new, smarter report function call
    # For now, we use the old one but we *should* update utils.py
    # to accept the new AI recommendations.
